...
- **Phase 5 improvements**: Redirect handling, activity name cleanup, folder expansion, type-aware dispatch, debug logging, progress bar fixes, onetopic section names, label content + activity descriptions, semester grouping with state migration, state save bug fix, format-grid multi-page section fetching, format-onetopic multi-tab fetching, modtype CSS class type detection, balanced-div altcontent parser, Moodle 4.x fp-filename span variant, duplicate folder name deduplication, Pass 15–17 (339/339 tests): reset bug fixes (sidecarPath tracking, recursive removeEmptyDirs, --dry-run tree output), course listing improvements (perpage=200, two-step parsing), SHA-256 hashing throughout (atomicWrite→downloader→incremental), iframe-embed fix, course README.md from summary, forum deep-dive (all discussion threads), assignment feedback + grades + submission downloads, file-checker exit-2 blocking, Pass 18 (365/365 tests): TUI + User-Files-Move, Pass 19: TUI + SK Refinements (dead code removal, SK folder structure fix, full-screen rendering, cursor management), Pass 20: Courses AJAX + stderr fix (HWR Moodle AJAX course listing for 42 courses, df stderr pipe to prevent terminal noise, TUI exit silence), Pass 21 (368/368 tests): Scrape bug fixes (.description.md sidecar handling, sync count clarification, resourceworkaround popup-link pattern, legacy mis-classification auto-promotion, data-activityname parsing for customcert cross-references), Pass 22 (377/377 tests): Section Descriptions (extracted section-level summaries `<div class="summarytext">` via depth-counter balancing, 38 `_Abschnittsbeschreibung.md` files written across courses including GPM course greeting and RTG introductions), Pass 23–25 (384/384 tests): Output quality refinements, assignment submission extraction, ENAMETOOLONG resolution for long filenames, Pass 26–28 (393/393 tests): Online text submissions, info-md always re-fetch, .jpeg double-extension fix, folder description extraction, sidecar deduplication filter with short-description consolidation, Pass 29 (408/408 tests): &nbsp; filename sanitisation fix, empty _Abschnittsbeschreibung filtering, smart folder subfolders (>1 folder → subfolder per folder with _Ordnerbeschreibung.md), onetopic data-number tab fix, sidecar batch content dedup, embedded image download (pluginfile.php images → images/ subfolder with URL rewriting), orphan marking for renamed resources, scrape/reset count reconciliation with categorized breakdown, Pass 30 (452/452 tests): YouTube extraction enhancements (Pattern 5: `data-embed-frame` for `filter_youtube_sanitizer`, `youtube-nocookie.com` domain support), onetopic sectionId collision fix (`s${sectionNum}` instead of `s${sectionNum-1}`), file organization quality improvements across VdZ, Betriebssystempraxis, DevOps, FRbüro, PM, and Bibliothek courses, Pass 31 (471/471 tests): Label subfolder fixes (single-divider safety `<2`, folder-description exclusion, (Kopie) strip, compound nesting, divider skip, separator skip), sidecar short-description dedup + label-md cross-dedup, headingless table conversion (custom Turndown rule for `<td>`-only tables), DevOps course name regex fix (`[A-Z]{0,6}` for cohorts without suffix), Pass 32 (483/483 tests): Icon-heading subfolder detection (`extractIconHeadingText()` + `hasSmallIcon()` for decorative icon + heading labels, WissArb Stichwort and Selbstmanagement subfolder grouping), Pass 33 (492/492 tests): Content-rich divider preservation (`isDividerContentRich()` — dividers with substantial content beyond heading/icon/attribution are written as `_SubfolderName.md` inside their subfolder instead of being dropped; fixes WissArb Lernziele, VdZ Einführung content loss), Pass 34 (512/512 tests): Feature gap cleanup — change report after scrape (new/updated file list with `+`/`~` prefixes), graceful Ctrl+C with AbortController + partial state save, state file `.bak` backup with auto-recovery from corruption, `msc clean` tip in status output, unknown activity type logging (`DownloadPlanResult.unknownTypes` + end-of-scrape summary), retry jitter (full jitter `[baseDelay/2, baseDelay*1.5]` to avoid thundering herd), Pass 35 (514/514 tests): Info-md 24-hour cache (promotion logic from "always re-fetch info-md assignments/feedback/etc" to "only re-fetch if state entry lastModified >24h old", fixing 99 unnecessary re-downloads on consecutive runs), Pass 54 (848/848 tests): `excludePaths` config key + `picomatch` dep, glob exclusion in `collectFiles` (built-in `.claude/**` and `.git/**` patterns + user patterns via picomatch), TUI config pre-fill (readline buffer insertion for in-place editing), `excludePaths` list editor screen with built-in + custom patterns
