...
### TICKET-8: No credential persistence on Linux — every run requires manual re-entry

| Field | Value |
|-------|-------|
| Type | feature |
| Severity | high |
| Affected command | `msc scrape`, `msc auth set` |
| Persona(s) | persona-02-tobias |
| Gap reference | Gap 1 |

**Description:** On Linux, keytar is not available and there is no fallback persistent credential store. Every invocation of `msc scrape` prompts for username and password interactively. This makes any form of automation (cron, scripts) impossible.
...
### TICKET-9: `--non-interactive` is broken on Linux
...
**Proposed fix:** Implement env-var credential injection (TICKET-8) first. Once `MSC_USERNAME`/`MSC_PASSWORD` env vars are supported, `--non-interactive` will work on Linux when they are set. Update `--help` to document the env-var path.

---

### TICKET-10: `msc auth status` misleadingly reports "system keychain (keytar)" on Linux

| Field | Value |
|-------|-------|
| Type | bug |
| Severity | medium |
| Affected command | `msc auth status` |
| Persona(s) | persona-02-tobias |
| Gap reference | Gap 2 |
...
### TICKET-11: `msc auth set` silently fails on Linux without warning

| Field | Value |
|-------|-------|
| Type | bug |
| Severity | high |
| Affected command | `msc auth set` |
| Persona(s) | persona-02-tobias |
| Gap reference | Gap 3 |
...
**Proposed fix:** When keytar is unavailable, `msc auth set` should immediately print a clear warning before prompting: `Warning: Persistent credential storage is not available on this platform (keytar not installed). Credentials will not be saved between runs.` and exit early rather than collecting credentials that will be discarded. Or better: redirect the user to the env-var approach (TICKET-8).

---

### TICKET-12: No targeted orphan-cleanup command
...
### TICKET-13: `msc status --issues` tip is shown when orphans exist but no user-added files
...
**Description:** When `msc status --issues` shows orphaned files but zero user-added files, the output still shows "Tip: Run `msc clean`..." — but `msc clean` does not handle orphans. This tip is irrelevant and potentially misleading.

**Proposed fix:** Show the `msc clean` tip only when user-added files > 0. When orphans > 0, show an appropriate tip such as: `Tip: Run \`msc reset\` to remove orphaned files and refresh state.` (Or update the tip once TICKET-12 is implemented.)
