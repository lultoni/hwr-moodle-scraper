...
**Description:** On macOS, `.md` files open in TextEdit by default, showing raw Markdown syntax. Beginners and non-technical users see `##`, `**`, and `[[` rather than formatted content and assume the files are broken.

**Proposed fix:** Add a one-time post-scrape notice explaining that `.md` files are Markdown, recommend an app (Typora, Obsidian, VS Code), and/or generate a `_HOW_TO_OPEN_FILES.md` in the output root. This affects all non-technical macOS users.
...
**Description:** `.url.txt` files contain a plain-text URL. macOS does not associate `.url.txt` with any browser, so double-clicking opens TextEdit. Users must manually copy the URL to a browser. This is a significant friction point for all non-technical users.

**Proposed fix:** Generate `.webloc` files (macOS) alongside or instead of `.url.txt`. A `.webloc` file is an XML plist that macOS recognises as a browser-openable bookmark. Windows equivalent is `.url`. Consider platform detection at scrape time to generate the appropriate format, while keeping `.url.txt` as a portable fallback.

---

### TICKET-14: Raw German Moodle error messages surfaced untranslated in CLI output
...
### TICKET-15: Generated filenames `_Abschnittsbeschreibung.md` are in German — confusing to non-German users
...
**Description:** The change report after a scrape lists new and updated files with `+` and `~` prefixes but provides no legend. Users must guess or infer the meaning.
