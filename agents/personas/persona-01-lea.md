...
### TICKET-1: `.md` files open as raw text on macOS — not readable by beginners

| Field | Value |
|-------|-------|
| Type | ux |
| Severity | high |
| Affected command | output-files (all `.md` output) |
| Persona(s) | persona-01-lea |
| Gap reference | Gap 5 |

**Description:** On macOS, `.md` files open in TextEdit by default, showing raw Markdown syntax (`##`, `**`, `[[`) rather than rendered text. Beginners expect formatted content and are confused or alarmed.

**Proposed fix:** Add a one-time notice after the first scrape explaining that `.md` files are Markdown and can be opened with apps like Typora, Obsidian, or VS Code. Alternatively, generate a `_HOW_TO_OPEN_FILES.md` in the output root with brief guidance. A macOS `open -a TextEdit` default cannot be changed by the CLI, but the README and post-scrape output can guide users.

---

### TICKET-2: `.url.txt` files are not openable by double-click

| Field | Value |
|-------|-------|
| Type | ux |
| Severity | high |
| Affected command | output-files (url activities → `.url.txt`) |
| Persona(s) | persona-01-lea |
| Gap reference | Gap 6 |

**Description:** `.url.txt` files contain a URL as plain text. Double-clicking opens TextEdit, not a browser. The user must manually copy the URL. On macOS, `.webloc` files open in the default browser; Windows has `.url` files for the same purpose.

**Proposed fix:** Consider generating `.webloc` (macOS) or `.url` (Windows) files instead of `.url.txt`, or in addition to it. Alternatively, add a post-install tip explaining how to open `.url.txt` files. This is a platform-specific UX gap that affects all non-technical users.

---

### TICKET-3: No README or orientation file in output folder root

| Field | Value |
|-------|-------|
| Type | docs |
| Severity | medium |
| Affected command | scrape (first run) |
| Persona(s) | persona-01-lea |
| Gap reference | Gap 7 |

**Description:** The output folder (`~/moodle-files/`) contains only course subfolders. A first-time user opening it in Finder has no guide explaining what the folder structure means, what file types are present, or how to open `.md` and `.url.txt` files.

**Proposed fix:** On first scrape, write a `_README.md` or `_ABOUT.md` to the output root explaining: the folder structure (`Semester_N/CourseName/Section/`), the file types (`.pdf`, `.md`, `.url.txt`), and how to open `.md` files. Track it in `generatedFiles`. Refresh on each run.

---

### TICKET-4: "Sidecars" and other jargon in `Done:` summary not explained
...
**Description:** The scrape summary line `Done: 847 downloaded, 193 sidecars, 0 skipped.` uses the word "sidecars" — an internal concept with no meaning to users. "Skipped" is also unexplained.

**Proposed fix:** Replace "sidecars" with a user-friendly term such as "description files" or "notes". Add a brief parenthetical or follow-up hint on first run: `193 description files (activity notes saved as .md)`. Alternatively, fold sidecars into the main count and surface them only in `--verbose` mode.

---

### TICKET-5: No estimated time or progress detail during slow first scrape
...
**Description:** During a first-run scrape of 12+ courses, the progress bar can stall visually on a slow course for 30–60 seconds. There is no indication of what the tool is doing or how long it will take. Beginners often assume the tool has crashed.

**Proposed fix:** Show the current course name and activity being processed in the progress bar label (e.g. `[7/12] Mathematik — Blatt 3.pdf`). A rough ETA (even "~3 min remaining") would also help. The current bar already shows course names — ensure it updates frequently enough on large courses.

---

### TICKET-6: Change report prefix symbols have no legend
...
**Proposed fix:** Add a one-line legend before the file list: `Legend: + new  ~ updated`. This takes one line and removes all ambiguity.

---

### TICKET-7: TUI Config screen shows raw config keys without descriptions
...
**Proposed fix:** Add a short description column or tooltip-style hint line for each config key in the TUI config screen. For example: `maxConcurrentDownloads — How many files to download at once (default: 5, range: 1–20)`.
