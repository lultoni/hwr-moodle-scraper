...
- **After changes**: run `npx vitest run` — all 894 tests must pass; commit with a contextual message
...
- All 22 timeline steps fully implemented; 894/894 tests passing (50 test files)
- Full CLI: auth, scrape, status, wizard, reset, tui + first-run wizard + clean
- 57 cleanup/improvement passes completed + 1 bug fix pass; current version: `0.10.0`
