...
- **After changes**: run `npx vitest run` — all 873 tests must pass; commit with a contextual message
...
- All 22 timeline steps fully implemented; 873/873 tests passing (50 test files)
- Full CLI: auth, scrape, status, wizard, reset, tui + first-run wizard + clean
- 56 cleanup/improvement passes completed + 1 bug fix pass; current version: `0.10.0`
