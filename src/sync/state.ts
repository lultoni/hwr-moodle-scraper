...
export interface State {
  ...
  /**
   * Flat list of file paths generated by the scraper outside the per-course FileState
   * system — currently: course README.md files and section _SectionDescription.md
   * files. These are written on every run from live Moodle HTML (not tracked per-resource)
   * and must be included in knownPaths so msc status and msc reset treat them as scraper-
   * owned rather than user-added files.
   *
   * Rule: any file written by runScrape that is NOT stored in a FileState entry MUST be
   * added to this list, otherwise it will appear as a "user file" in msc status and will
   * not be deleted by msc reset.
   */
  generatedFiles?: string[];
  /**
   * Record of files changed in the most recent scrape run. Persisted so that
   * `msc status --changed` can replay the change report after the terminal closes.
   */
  lastSync?: LastSyncRecord;
}

/** Files changed in a single scrape run — stored for `msc status --changed`. */
...
export type PartialState = { courses: Record<string, Partial<CourseState>>; generatedFiles?: string[]; lastSync?: LastSyncRecord };
...
export class StateManager {
  ...
  async save(data: PartialState): Promise<void> {
    const state: State = {
      ...
      ...(data.lastSync ? { lastSync: data.lastSync } : {}),
    };
